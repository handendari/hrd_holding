    ...
    $.ajax({
        ...
        success: function (dt) {
            //if (dt.listTrn != null && dt.listTrn.length > 0) {
                f_FillTableIns(dt.listIns);
            //}
        }
    });
}

function f_FillTableIns(listIns) {
    vDataIns.length = 0;
    for (var i = 0; i < listIns.length; i++) {
        var row = {};
        row["employment_code"] = listIns[i].employee_code;
        row["seq_no"] = listIns[i].seq_no;
        row["organizer"] = listIns[i].organizer;
        row["material"] = listIns[i].material;
        row["place"] = listIns[i].place;
        row["start_date"] = new Date(parseInt(listIns[i].start_date.substr(6)));
        row["end_date"] = new Date(parseInt(listIns[i].end_date.substr(6)));
        row["company"] = listIns[i].company;
        row["chk_company"] = listIns[i].chk_company;
        row["value"] = listIns[i].value;

        vDataIns.push(row);
    }

    var vAdapter = new $.jqx.dataAdapter(vSrcIns);
    $("#tblInstitutions").jqxGrid({ source: vAdapter });
}

function f_EmptyInsDetail() {
...
function f_DeleteEmployeeIns(pEmpCode) {
    $("#modYesNo").jqxWindow('close');
    f_ShowLoaderModal();

    var selectedRowIndex = $("#tblInstitutions").jqxGrid('selectedrowindex');
    var vSeqNo = $('#tblInstitutions').jqxGrid('getcellvalue', selectedRowIndex, "seq_no");
...
$(document).ready(function () {
    //#region INIT FAMILY
    $("#btnInstitutionsNew").jqxButton({ theme: vTheme });
    $("#btnInstitutionsEdit").jqxButton({ theme: vTheme });
    $("#btnInstitutionsDelete").jqxButton({ theme: vTheme });

    //#region Table TRAINING
    function initGridInstitutions() {
        $("#tblInstitutions").jqxGrid(
        {
            width: '100%',
            height: 200,
            theme: vTheme,
            columnsresize: true,
            rowsheight: 25,
            source: new $.jqx.dataAdapter(vSrcIns),
            columns: [
